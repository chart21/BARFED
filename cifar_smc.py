...
from smc_utils import weight_averaging as wa
...
is_organized = True
hostile_node_percentage = 0.00 #malicious participant ratio
byzantine_mean =0
byzantine_std =1
...
for iteration in range(iteration_num):
    ...
    tn.start_train_end_node_process_byzantine_for_cifar_with_augmentation(number_of_samples, x_train_dict, y_train_dict,
                                                                          ...
                                                                          byzantine_std,
                                                                          device, clipping,clipping_threshold, iteration_byzantine_seed)
    ...
    if weight_averaging:
        if smc:
            main_model = wa.get_averaged_weights_with_smc(model_dict, device)
        else:
            main_model = wa.get_averaged_weights_without_smc(model_dict, device)
    
    
    
    
    #main_model = tn.strict_condition_without_outliers_set_averaged_weights_as_main_model_weights_and_update_main_model(main_model,   model_dict, iteration_distance, device)
